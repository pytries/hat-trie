...
def test_getitem_set():
    trie = hat_trie.FloatTrie()
    trie['foo'] = 5.1
    trie['bar'] = 10.1
    assert abs(trie['foo'] - 5.1) < .001
    assert abs(trie['bar'] - 10.1) < .001
    ...
    non_ascii_key = 'вася'
    trie[non_ascii_key] = 20.1
    assert abs(trie[non_ascii_key] - 20.1) < .001

def test_get():
    trie = hat_trie.FloatTrie()

    assert isnan(trie.get('foo'))
    assert isnan(trie.get('bar'))
    assert abs(trie.get('foo', 5.0) - 5.0) < .001

    trie['foo'] = 5.5
    trie['bar'] = 10.1

    assert abs(trie.get('foo') - 5.5) < .001
    assert abs(trie.get('bar') - 10.1) < .001
